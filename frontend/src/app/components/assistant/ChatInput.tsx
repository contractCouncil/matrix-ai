...
import type { MatrixAIDocument, MatrixAIMessage } from "../shared/types";

export interface ChatInputHandle {
    addDoc: (doc: MatrixAIDocument) => void;
}

interface Props {
    onSubmit: (message: MatrixAIMessage) => void;
    onCancel: () => void;
    isLoading: boolean;
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
    ...
    const [attachedDocs, setAttachedDocs] = useState<MatrixAIDocument[]>([]);
    ...
    const handleAddDocFromProject = useCallback((doc: MatrixAIDocument) => {
        setAttachedDocs((prev) => {
            if (prev.some((d) => d.id === doc.id)) return prev;
            return [...prev, doc];
        });
