...
export function ProjectPage({ projectId }: Props) {
    ...
    const [folders, setFolders] = useState<MatrixAIFolder[]>([]);
    const [chats, setChats] = useState<MatrixAIChat[]>([]);
    ...
    const [uploadVersionDoc, setUploadVersionDoc] =
        useState<MatrixAIDocument | null>(null);
    const [viewingDoc, setViewingDoc] = useState<MatrixAIDocument | null>(null);
    ...
    const [versionsByDocId, setVersionsByDocId] = useState<
        Map<string, MatrixAIDocumentVersion[]>
    >(() => new Map());
     ...
     */
    function handleUploadNewVersion(doc: MatrixAIDocument) {
        setUploadVersionDoc(doc);
    }

    async function submitNewVersion(
        doc: MatrixAIDocument,
        file: File,
        displayName: string,
    ) {
        try {
    ...
    useEffect(() => {
        Promise.all([
            ...
            listProjectChats(projectId).catch(() => [] as MatrixAIChat[]),
    ...
    async function handleCreateFolder(parentId: string | null) {
        ...
        const optimistic: MatrixAIFolder = {
            id: tempId,
    ...
    function handleDocsSelected(newDocs: MatrixAIDocument[]) {
        setProject((prev) =>
            prev ? {
                ...prev,
                documents: [
