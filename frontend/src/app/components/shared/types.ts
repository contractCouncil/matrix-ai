...
export interface MatrixAIFolder {
  id: string;
  project_id: string;
  user_id: string;
  name: string;
...
export interface MatrixAIProject {
  id: string;
  user_id: string;
  is_owner?: boolean;
  name: string;
...
export interface MatrixAIDocument {
  id: string;
  user_id?: string;
...
export interface MatrixAIEditAnnotation {
  type?: "edit_data";
  kind?: "edit";
  edit_id: string;
  document_id: string;
...
export type AssistantEvent =
  ...
  | {
        ...
        version_number?: number | null;
        download_url: string;
        annotations: MatrixAIEditAnnotation[];
        error?: string;
        isStreaming?: boolean;
    }
  | { type: "content"; text: string; isStreaming?: boolean };

export interface MatrixAIMessage {
  role: "user" | "assistant";
  content: string;
 ...
 */
export interface MatrixAICitationAnnotation {
  type: "citation_data";
  ref: number;
  doc_id: string;
  document_id: string;
...
export function formatCitationPage(a: MatrixAICitationAnnotation): string {
  if (typeof a.page === "string") return `Page ${a.page}`;
  return `Page ${a.page}`;
}

/** Produce a reader-friendly version of the quote (replaces [[PAGE_BREAK]] with "..."). */
export function displayCitationQuote(a: MatrixAICitationAnnotation): string {
...
export interface MatrixAIWorkflow {
  id: string;
  user_id: string | null;
  title: string;
...
export interface MatrixAIChatDetailOut {
  chat: MatrixAIChat;
  messages: MatrixAIMessage[];
}
